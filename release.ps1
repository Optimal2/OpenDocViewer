...
function Exec {
  ...
  # Suppress benign stderr when exit code == 0 (avoids CRLF/LF noise)
...
function ExecNpm {  # run npm reliably on Windows via cmd.exe
  param([string[]]$NpmArgs,[string]$Cwd,[switch]$Quiet)
  $cmd = $env:ComSpec; if(-not $cmd){ $cmd='cmd.exe' }
  $allArgs = @('/d','/c','npm') + $NpmArgs
  Exec -File $cmd -Args $allArgs -Cwd $Cwd -Quiet:$Quiet
}
...
if(-not [string]::IsNullOrWhiteSpace($porcelain)){
  ...
  switch($ch.ToUpper()){
    'A'{ $null = Exec 'git' @('add','-A') -Cwd $repoRoot; $null = Exec 'git' @('commit','-m','chore(release): pre-release housekeeping') -Cwd $repoRoot -AllowNonZero }
...
# Version bump via package.json scripts (silent object return; keep npm output)
...
try{
  if($hasRemote){ $null = Exec 'git' @('push','origin',$branch,'--follow-tags') -Cwd $repoRoot }
  ...
  $stashVar = Get-Variable -Name __REL_STASH__ -Scope Global -ErrorAction SilentlyContinue
  if($stashVar -and $stashVar.Value){
    Write-Host "Restoring stashed changes..."
    $null = Exec 'git' @('stash','pop') -Cwd $repoRoot -AllowNonZero
    $global:__REL_STASH__ = $null
  }
}

# Post-push verification: confirm head & tag on origin
if($hasRemote){
  $null = Exec 'git' @('fetch','--tags') -Cwd $repoRoot -Quiet
