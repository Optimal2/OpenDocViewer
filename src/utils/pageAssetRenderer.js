...
export class PageAssetRenderer {
  ...
  /**
   * Render a large set of PDF pages through long-lived worker batch jobs. This is intended for
   * eager warm-up runs where the full PDF page count is already known; interactive single-page
   * requests should keep using renderPageAsset so they can retain priority ordering.
   *
   * @param {Array<Object>} descriptors
   * @param {Object=} options
   * @param {function(Object): void=} options.onItemResult
   * @param {number=} options.rendersPerWorker
   * @param {string=} options.batchMode
   * @param {number=} options.thumbnailMaxWidth
   * @param {number=} options.thumbnailMaxHeight
   * @param {number=} options.fullPageScale
   * @returns {Promise<{itemCount:number,successCount:number,errorCount:number,workerCount:number,batchMode:string}>}
   */
  async renderPdfPageAssetBatch(descriptors, options = {}) {
    if (!this.shouldTryPdfWorker()) {
      throw new Error('PDF worker batch rendering is not available.');
    }
    ...
    if (!items.length) {
      return {
        ...
        successCount: 0,
        errorCount: 0,
        workerCount: Math.max(0, Number(this.pdfWorkerPool?.getWorkerCount?.() || 0)),
        batchMode: 'partitioned',
      };
    }
    ...
    const getSourceBlob = async (sourceKey) => {
      ...
      if (!sourceBlobCache.has(key)) {
        const blob = await this.tempStore.getBlob(key);
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      const sourceBlob = await getSourceBlob(descriptor.sourceKey);
      if (!sourceBlob) {
        batchItems.push({
          itemId: index,
          descriptor,
          missingSourceBlob: true,
          payload: null,
        });
        continue;
      }
      ...
      batchItems.push({
        ...
        payload: {
          ...
          variant: options?.variant === 'thumbnail' ? 'thumbnail' : 'full',
    ...
    const usableItems = batchItems.filter((item) => item && !item.missingSourceBlob && item.payload);
    const workerCount = Math.max(1, Number(this.pdfWorkerPool?.getWorkerCount?.() || 1));
    const rendersPerWorker = Math.max(1, Math.min(8, Math.floor(Number(options?.rendersPerWorker) || 1)));
    const batchMode = String(options?.batchMode || 'partitioned').toLowerCase() === 'queue' ? 'queue' : 'partitioned';
    let successCount = 0;
    let errorCount = 0;

    for (const missing of batchItems.filter((item) => item?.missingSourceBlob)) {
      errorCount += 1;
      try {
        options?.onItemResult?.({
          ...
          durationMs: 0,
          error: `Missing source blob for worker render ${missing.descriptor?.sourceKey}`,
        });
      } catch {}
    }

    const onItemResult = (result) => {
      const itemId = Math.max(0, Number(result?.itemId) || 0);
      const original = batchItems[itemId] || {};
      if (result?.ok) {
        successCount += 1;
      ...
      this.recordPdfWorkerFallback(result?.errorDetails || new Error(String(result?.error || 'PDF worker batch item failed')), original.descriptor, options?.variant || 'full');
      try {
        options?.onItemResult?.({
          ok: false,
          descriptor: original.descriptor,
    ...
    const partitions = batchMode === 'queue'
      ? usableItems.map((item) => [item])
      : partitionContiguous(usableItems, workerCount);

    await Promise.all(partitions.map((partition) => this.pdfWorkerPool.renderBatch(partition, {
