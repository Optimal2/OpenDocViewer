  ...
  try {
    ...
    for (let i = job.pageStartIndex + 1; i <= job.pageStartIndex + job.pagesInvolved; i++) {
      ...
      canvas.toBlob(blob => {
        ...
        logger.debug(`Main thread processed PDF page`, { url, fileIndex: job.index, pageIndex: i - 1 });
        handleWorkerMessage({
          data: {
            jobs: [{
              fullSizeUrl: url,
        ...
        }, insertPageAtIndex, sameBlob, isMounted);
      }, 'image/png');
    }
  } catch (error) {
...
export const renderTIFFInMainThread = async (job, insertPageAtIndex, sameBlob, isMounted) => {
  try {
    ...
    for (let i = job.pageStartIndex; i < job.pageStartIndex + job.pagesInvolved; i++) {
      ...
      logger.info(`Processing TIFF page ${i}, Compression: ${compression}, PhotometricInterpretation: ${photometricInterpretation}, BitsPerSample: ${bitsPerSample}, SamplesPerPixel: ${samplesPerPixel}, PlanarConfiguration: ${planarConfiguration}, ExtraSamples: ${extraSamples}, File Index: ${job.index}, Page Index: ${i}, All Pages Index: ${job.allPagesStartingIndex + (i - job.pageStartIndex)}`);
      ...
      if (blob) {
        logger.debug(`Main thread processed TIFF page`, { url, fileIndex: job.index, pageIndex: i });
        insertPageAtIndex({
          fullSizeUrl: url,
