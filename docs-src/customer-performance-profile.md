...
This is independent of the document-loading mode below: every page targets a physical print
resolution (`targetDpi`, default 300) instead of a fixed PDF-point multiplier, independent of the
screen. A4 at 300 dpi uses 4.167 (2480 × 3509 pixels); only the safety caps lower the factor, for
very large pages. Larger rasters cost more memory and render time. The 40 MP per-page cap is halved on the low memory tier, but is not a session-wide memory
