...
| `useDocumentViewer~activateComparePane` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1421` | Open compare mode when needed and make the right pane the default target. |
| `useDocumentViewer~activatePrimaryPane` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1413` |  |
| `useDocumentViewer~applyThumbnailWidth` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1523` | Mouse down handler for the thumbnail resizer; listens for mousemove/up on window. |
| `buildDocumentSelectionModel` | function | `src/components/DocumentViewer/useDocumentViewer.js:259` |  |
| `buildImageRotationDependencyKey` | function | `src/components/DocumentViewer/useDocumentViewer.js:103` |  |
| `buildVisibleDocumentNavigationModel` | function | `src/components/DocumentViewer/useDocumentViewer.js:337` | Build the visible-document grouping used by document-level navigation. |
| `clampPage` | function | `src/components/DocumentViewer/useDocumentViewer.js:45` | Clamp a 1-based page number into \[1, total\]. |
| `useDocumentViewer~closeCompare` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1449` | Close compare mode without affecting the left page. |
...
| `useDocumentViewer~getDocumentNavigationState` | constant | `src/components/DocumentViewer/useDocumentViewer.js:641` | Resolve document-navigation state for the requested pane. |
| `useDocumentViewer~getImagePropertiesSetter` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1475` |  |
| `getPageDocumentNavigationMeta` | function | `src/components/DocumentViewer/useDocumentViewer.js:311` |  |
| `useDocumentViewer~goToFirstDocument` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1174` | Jump to the first page of the first visible document. |
| `useDocumentViewer~goToFirstPage` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1130` | Jump to the first visible page in the requested target pane. |
| `useDocumentViewer~goToLastDocument` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1186` | Jump to the first page of the last visible document. |
| `useDocumentViewer~goToLastPage` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1139` | Jump to the last visible page in the requested target pane. |
| `useDocumentViewer~goToNextDocument` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1162` | Jump to the first page of the next visible document. |
| `useDocumentViewer~goToNextPage` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1121` | Move one page forward in the requested target pane. |
| `useDocumentViewer~goToPreviousDocument` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1150` | Jump to the first page of the previous visible document \(or to the current document start when the active pane already points inside the first visible document\). |
| `useDocumentViewer~goToPreviousPage` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1112` | Move one page backward in the requested target pane. |
| `useDocumentViewer~handleBrightnessChange` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1489` |  |
| `useDocumentViewer~handleCompare` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1431` | Toggle compare mode. |
| `useDocumentViewer~handleContrastChange` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1497` |  |
| `useDocumentViewer~handlePageNumberChange` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1028` | Change the primary page using an original page number \(or a visible-page updater function when called from navigation helpers\). |
| `useDocumentViewer~handlePrimaryDisplayStateChange` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1076` | Keep requested-page state and the actually displayed page synchronized for diagnostics. |
| `useDocumentViewer~handleVisiblePageNumberChange` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1037` | Change the primary page by a visible page number from the thumbnail strip. |
| `hasExcludedPages` | function | `src/components/DocumentViewer/useDocumentViewer.js:128` |  |
| `useDocumentViewer~hideDocumentFromSelection` | constant | `src/components/DocumentViewer/useDocumentViewer.js:904` | Immediately exclude every page that belongs to the same document as the provided original page index. |
| `useDocumentViewer~hidePageFromSelection` | constant | `src/components/DocumentViewer/useDocumentViewer.js:866` | Immediately exclude a page from the active selection and apply the filtered session. |
| `ImageProperties` | typedef | `src/components/DocumentViewer/useDocumentViewer.js:383` | Image adjustment properties for canvas edit mode. |
...
| `normalizeViewerPaneTarget` | function | `src/components/DocumentViewer/useDocumentViewer.js:411` | Normalize any pane key into the viewer&#39;s two supported navigation targets. |
| `<anonymous>~onMove` | function | `src/components/DocumentViewer/useDocumentViewer.js:1588` |  |
| `resolveDocumentSelectionPageNumber` | function | `src/components/DocumentViewer/useDocumentViewer.js:242` |  |
...
| `useDocumentViewer~resolveNearestVisibleOriginalPageNumber` | constant | `src/components/DocumentViewer/useDocumentViewer.js:719` |  |
| `resolveProposedVisiblePageNumber` | function | `src/components/DocumentViewer/useDocumentViewer.js:230` | Resolve either a direct visible-page value or a React setState-style updater function. |
| `useDocumentViewer~resolveTargetOriginalPageNumber` | constant | `src/components/DocumentViewer/useDocumentViewer.js:980` | Resolve the next original 1-based page number from a visible-page update. |
| `useDocumentViewer~selectForCompare` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1464` | Select a page for the right-hand compare pane. |
| `useDocumentViewer~setActivePane` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1406` | Set the default pane for compare-aware navigation and editing actions. |
| `useDocumentViewer~setComparePageNumber` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1064` | Change the compare page using an original page number \(or a visible-page updater function when called from compare navigation helpers\). |
| `useDocumentViewer~setIsExpanded` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1394` | Setter for the editing controls visibility. |
| `useDocumentViewer~setVisibleComparePageNumber` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1050` | Change the compare page by a visible page number from the toolbar page field. |
| `useDocumentViewer~setZoomMode` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1300` | Set zoom mode directly \(&#39;FIT_PAGE&#39;\|&#39;FIT_WIDTH&#39;\|&#39;FIT_CUSTOM&#39;\|&#39;ACTUAL_SIZE&#39;\|&#39;CUSTOM&#39;\). |
| `useDocumentViewer~thumbnailSelectionPageNumber` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1104` | The thumbnail pane should react immediately when the user changes page. |
| `useDocumentViewer~toggleFitZoomMode` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1378` | Toggle between the two fit modes from the page surface. |
| `useDocumentViewer~updatePageTarget` | constant | `src/components/DocumentViewer/useDocumentViewer.js:1003` | Generic primary/compare page setter that accepts either a visible-page updater function or a concrete original page number. |
| `useDocumentViewer` | function | `src/components/DocumentViewer/useDocumentViewer.js:428` | Hook that centralizes viewer UI state and event handlers. |
| `ViewerPageTarget` | typedef | `src/components/DocumentViewer/useDocumentViewer.js:403` |  |
| `ZoomMode` | typedef | `src/components/DocumentViewer/useDocumentViewer.js:391` | Sticky zoom modes used by the viewer \(subset is used here\). |
| `ZoomState` | typedef | `src/components/DocumentViewer/useDocumentViewer.js:396` | Zoom state \(mode + current numeric scale\). |
