...
WHEN MATCHED THEN
    UPDATE SET HostId = source.HostId,
               ...
               -- ArtifactId is owned by the platform's artifact auto-apply and must never be assigned here.
               IsEnabled = source.IsEnabled,
...
WHEN MATCHED THEN
    UPDATE SET InstanceTemplateHostId = source.InstanceTemplateHostId,
               ...
               InstallPath = source.InstallPath,
               InstallationName = source.InstallationName,
               -- DesiredArtifactId is owned by the platform's artifact auto-apply and must never be assigned here.
               DesiredState = source.DesiredState,
               SortOrder = source.SortOrder,
               IsEnabled = source.IsEnabled,
               UpdatedUtc = SYSUTCDATETIME()
